...
jQuery.fn.getRepos = function (username) {
    ...
    $.gitUser(
        username,
        function (data) {
            var allRepos = data.data; /* JSON Parsing */
            var repos = allRepos.filter(filterForks).filter(filterArchived);
            /* console.log(repos); */

            try {
                sortByDateCreated(repos); /* Sorting by forks. You can customize it according to your needs. */
            } catch (err) {
            ...
            $(repos).each(function () {
                if ((this.name != (username.toLowerCase() + '.github.com'))) { /* Check for username.github.com repo */
                    list.append('<dt> \
                            <a style="font-size:20px;" target="_blank" href="' + (this.homepage ? this.homepage : this.html_url) + '"><h4 style="display: inline; padding-right: 2%;">/' + this.name + '   </h4></a> \
                            ...
                            <a href=' + this.html_url + '  target="_blank"><span class="numbertag">' + this.watchers + '</span></a> \
                            <span class="tag"><i class="fa fa-github fa-2" aria-hidden="true"></i> FORKS</span> \
                            <a href=' + this.html_url + '  target="_blank"><span class="numbertag">' + this.forks + '</span></a></div> \
                            <div style="padding-top: 2%;"><p>' + joypixels.shortnameToImage(this.description || '') + (this.homepage ? ('<a href="' + this.homepage + '"  target="_blank"> ' + this.homepage + '</a>') : "") + '</p></div> \
                        ');
    ...
    function sortByDateCreated(repos) {
        repos.sort(function (a, b) {
            /* Sort by created_at date. LIFO */
    ...
    function sortByForks(repos) {
        repos.sort(function (a, b) {
            /* Descending order for number of forks based sorting. */
            return b.forks - a.forks;
        });
    }

    function filterForks(repo) {
        return repo.fork === false;
    }

    function filterArchived(repo) {
